...
실제 실험 과정에서 블로그 포스트와 다른 점을 발견했습니다.

| 항목 | 블로그 내용 | 실제 실험 결과 |
|------|-----------|--------------|
| **JSONL 형식** | `"content": "문자열"` | `"content": [{"text": "문자열"}]` (Converse API 형식) |
| **system 필드** | 미언급 또는 messages 내부 | 최상위 `"system": [{"text": "..."}]` 배열 |
| **batchSize** | 32 (예시) | Nova Micro는 **max 1** (ValidationException) |
| **Fine-tuning 리전** | us-east-1/us-west-2 | **us-east-1만** fine-tuning 모델 노출 |
| **IAM description** | 한글 가능 | ASCII만 허용 (`[\u0020-\u007E]`) |
| **LoRA/PEFT 설정** | YAML 기반 상세 설정 | Bedrock API는 `hyperParameters` map만 지원 |
| **학습 시간** | 미언급 | Nova Micro 500 samples: **~4시간** (프로비저닝 포함) |
| **Provisioned Throughput** | 미언급 | 커스텀 모델 추론 시 필수, 쿼터 사전 신청 필요 |
| **Custom Model Deployment** | 미언급 | PT 대안: `create-custom-model-deployment` (on-demand, 쿼터 불필요) |

### 핵심 교훈

1. **Nova 전용 JSONL 형식**: 다른 Bedrock 모델(Claude, Titan 등)과 형식이 다름. Converse API 스타일 사용
2. **하이퍼파라미터 제한**: 모델별로 허용 범위가 다름. dry-run으로 사전 검증 필수
3. **리전 제한**: fine-tuning 지원 여부는 `customizationsSupported` 필드로 확인
